...
export class ConfigHelper extends EventEmitter {
  ...
  private sanitizeModelSelection(
    ...
    if (provider === "openai") {
      ...
      if (!allowedModels.includes(model)) {
        console.warn(
          `Invalid openrouter model specified: ${model}. Using default model: google/gemini-2.0-flash-exp:free`
        );
  ...
  public updateConfig(updates: Partial<Config>): Config {
    try {
      ...
      if (updates.apiKey && !updates.apiProvider) {
        ...
        } else if (updates.apiKey.trim().startsWith("org- || sk-or-")) {
          provider = "openrouter";
      ...
      if (
        ...
        if (updates.apiProvider === "openai") {
          ...
          updates.extractionModel = "deepseek/deepseek-chat-v3-0324:free";
          updates.solutionModel = "deepseek/deepseek-chat-v3-0324:free";
          updates.debuggingModel = "deepseek/deepseek-chat-v3-0324:free";
        } else {
  ...
  public isValidApiKeyFormat(
    ...
    if (provider === "openai") {
      ...
      return (
        /^org-[a-zA-Z0-9]{32,}$/.test(apiKey.trim()) ||
        /^sk-or-[a-zA-Z0-9]{32,}$/.test(apiKey.trim())
      );
    }
  ...
  public async testApiKey(
    ...
    if (!provider) {
      if (apiKey.trim().startsWith("sk-") || apiKey.trim().startsWith("org-")) {
        if (apiKey.trim().startsWith("sk-ant-")) {
          provider = "anthropic";
  ...
  private async testOpenRouterKey(
    ...
    try {
      // Make a simple GET request to the OpenRouter models endpoint
      ...
      if (response.ok) {
        ...
        return {
          valid: false,
          error:
            "Rate limit exceeded or insufficient quota for OpenRouter API key.",
        };
      } else {
        return {
          valid: false,
