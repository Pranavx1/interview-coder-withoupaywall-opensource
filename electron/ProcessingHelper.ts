...
export class ProcessingHelper {
    ...
    try {
      ...
      if (
        ...
        if (config.apiKey) {
          ...
          console.log(
            `${
              config.apiProvider === "openrouter" ? "OpenRouter" : "OpenAI"
  ...
  public async processScreenshots(): Promise<void> {
    ...
    if (
      config.apiProvider === "openai" ||
      (config.apiProvider === "openrouter" && !this.openaiClient)
    ) {
      this.initializeAIClient();

      if (!this.openaiClient) {
        console.error("OpenAI/OpenRouter client not initialized");
  ...
  private async processScreenshotsHelper(
    ...
    try {
      ...
      if (
        ...
        // Use OpenAI/OpenRouter for processing
        const messages = [
          {
        ...
        // Send to OpenAI/OpenRouter Vision API
        ...
        try {
          ...
          // Handle when OpenAI/OpenRouter might wrap the JSON in markdown code blocks
          const jsonText = responseText.replace(/```json|```/g, "").trim();
          problemInfo = JSON.parse(jsonText);
        } catch (error) {
          console.error("Error parsing OpenAI/OpenRouter response:", error);
          return {
            success: false,
            error:
  ...
  private async processExtraScreenshotsHelper(
    ...
    try {
      ...
      if (
        ...
        if (mainWindow) {
          mainWindow.webContents.send("processing-status", {
            message: `Analyzing code and generating debug feedback with ${
              config.apiProvider === "openrouter" ? "OpenRouter" : "OpenAI"
            }...`,
            progress: 60,
          });
